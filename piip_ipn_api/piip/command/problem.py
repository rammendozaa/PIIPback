...
def add_problem_to_database(response, url):
    ...
    title = getValue(
        '<div class="title">\\s*' + problemNum + "\\. ([\\s\\S]*?)</div>", response.text
    )
    ...
    output = getValue(
        '<div class="section-title">\\s*Output\\s*</div>([\\s\\S]*?)</div><div class="sample-tests">',
        response.text,
    )
    ...
    sampleInput += getValue(
        '<div class="sample-test">([\\s\\S]*?)</div>\\s*</div>\\s*</div>', response.text
    )
    hint = getValue(
        '<div class="section-title">\\s*Note\\s*</div>([\\s\\S]*?)</div></div></div></div>',
        response.text,
    )
